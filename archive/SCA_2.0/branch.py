...
def kill_attr(leaves, br, Dk):
    """ Removes the points that have reached Dk
    """
    tree = KDTree(leaves)
    mask = np.array(leaves, dtype=bool)
    for i in sorted(tree.query_ball_point(br.segment, r=Dk), reverse=True):
        mask[i] = False
...
def Grow(leaves, start_br, Ar, Dk, Bl):
    ...
    if dist < Ar:
        ...
        new_direction = Sum2di(new_segm, leaves[num])
...
def Grow2(leaves, start_br, Ar, Dk, Bl):
    ...
    di_neighbors = []
    for nbr in tree.query_ball_point(start_br.segment, r=Ar):
        di = leaves[nbr]
...
def borders_reached(br, R, center):
    if square_dist(center, br.segment)< R:
        return False
    else:
        return True

def borders_reached_polygon(br):
    raise Exception('ERROR')
    
def prob(tree, val, collection, Ar, v=1):
    if  v == 1:
        num = len(collection)
